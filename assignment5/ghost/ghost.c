...
int main(int argc, char *argv[])
{
    ...
    //  to the top
    MPI_Irecv(&data[(DOMAINSIZE-1)*DOMAINSIZE+1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0, MPI_COMM_WORLD, &request);
    ...
    MPI_Send(&data[(DOMAINSIZE-2)*DOMAINSIZE+1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0, MPI_COMM_WORLD);
    MPI_Wait(&request, &status);

    //  to the left
    MPI_Irecv(&data[(DOMAINSIZE-1)+DOMAINSIZE], 1, data_ghost, rank_right, 0, MPI_COMM_WORLD, &request);
    ...
    MPI_Send(&data[DOMAINSIZE-2+DOMAINSIZE], 1, data_ghost, rank_right, 0, MPI_COMM_WORLD);
    MPI_Wait(&request, &status);
